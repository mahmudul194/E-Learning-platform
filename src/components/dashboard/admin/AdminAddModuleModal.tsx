...
import { FolderTree, X, Plus, Trash2, Sparkles } from "lucide-react";
...
export default function AdminAddModuleModal({
  ...
  const [lessons, setLessons] = useState([
    { title: "Introduction & LOD Setup", videoUrl: "https://vimeo.com/76979871", duration: "45m" },
    { title: "Grid & Parametric Framing", videoUrl: "https://vimeo.com/76979871", duration: "60m" },
  ]);
  ...
  const addLesson = () => setLessons([...lessons, { title: "", videoUrl: "https://vimeo.com/76979871", duration: "50m" }]);
  const removeLesson = (idx: number) => lessons.length > 1 && setLessons(lessons.filter((_, i) => i !== idx));
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 font-sans animate-fade-in overflow-y-auto">
      <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 space-y-4 shadow-2xl border border-slate-100 ring-1 ring-black/5 animate-scale-in my-8 text-xs sm:text-sm">
        ...
        {/* Selected Course Scope + Auto Duration Highlight */}
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input type="text" required value={moduleNo} onChange={(e) => setModuleNo(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:border-[#0077b6] font-mono font-bold focus:outline-none" />
            </div>
            <div className="col-span-2">
              <label className="font-bold text-slate-700 block mb-1">Module Title</label>
              <input type="text" required placeholder="e.g. Advanced Structural Rebar & Column Detailing" value={moduleTitle} onChange={(e) => setModuleTitle(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:border-[#0077b6] focus:outline-none" />
            </div>
          </div>
              ...
              {lessons.map((les, idx) => (
                <div key={idx} className="p-2 rounded-xl bg-slate-50 border border-slate-200 grid grid-cols-1 sm:grid-cols-12 gap-2 items-center text-xs">
                  ...
                  <div className="sm:col-span-2 flex items-center gap-1">
                    <input type="text" placeholder="50m" value={les.duration} onChange={(e) => updateLesson(idx, "duration", e.target.value)} className="w-full px-2 py-1.5 rounded-lg bg-white border border-slate-200 text-center font-mono font-bold text-[#0077b6] focus:outline-none" />
                    {lessons.length > 1 && <button type="button" onClick={() => removeLesson(idx)} className="text-slate-400 hover:text-rose-600 p-1 cursor-pointer"><Trash2 className="w-3.5 h-3.5" /></button>}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-slate-100">
            ...
            <button type="submit" className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#002b5b] to-[#0077b6] hover:from-[#001830] hover:to-[#005a8c] text-white font-extrabold shadow-md transition-all cursor-pointer hover:scale-102">Save & Publish ({autoDuration})</button>
          </div>
        </form>
      </div>
    </div>,
